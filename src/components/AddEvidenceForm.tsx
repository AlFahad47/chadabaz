...
interface AddEvidenceFormProps {
    pinId: string;
    onClose: () => void;
    onSubmitSuccess: (updatedPin: any) => void;
}

export default function AddEvidenceForm({ pinId, onClose, onSubmitSuccess }: AddEvidenceFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (photos.length === 0 && videoLinks.length === 0) {
            setError("Please provide at least one photo or video link.");
            return;
        }
        ...
        try {
            const response = await fetch(`/api/pins/${pinId}/evidence`, {
                method: "POST",
                headers: {
    ...
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto border border-zinc-200 dark:border-zinc-800">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">Add Evidence</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <input
                        type="text"
                        required
                        placeholder="e.g. Personal experience, News link..."
